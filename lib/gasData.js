...
class GasData {
  ...
  /**
   * Gets the total gas used by all known deployments, based on all registered gas data.
   * This only accounts for registered methods, thus not deployment transactions!
   */
  getGasUsed() {
    let gasUsed = 0;
    for (const deployment of this.deployments) {
      const start = deployment.gasDataStart;
      gasUsed += deployment.gasData.reduce(
        (a, b, i) => (i >= start ? a + b : a),
        0
      );
    }
    for (const methodId in this.methods) {
      const start = this.methods[methodId].gasDataStart;
      gasUsed += this.methods[methodId].gasData.reduce(
        (a, b, i) => (i >= start ? a + b : a),
        0
      );
    }
  ...
  /**
   * Make getGasUsed() ignore all existing gas data for all known deployments
   */
  rebaseGasUsed() {
    for (const deployment of this.deployments)
      deployment.gasDataStart = deployment.gasData.length;
    for (const methodId in this.methods)
      this.methods[methodId].gasDataStart = this.methods[
        methodId
      ].gasData.length;
  }
